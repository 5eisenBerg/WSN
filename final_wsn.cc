...
const double ALPHA = 0.7, BETA = 0.3, E_ELEC = 50e-9, E_FS = 10e-12, E_MP = 0.0013e-12,
             D0 = sqrt(E_FS / E_MP);
...
static uint64_t g_packetsGenerated = 0, g_packetsDelivered = 0, g_packetsDropped = 0,
                g_normalPacketsDelivered = 0, g_hpPacketsDelivered = 0;
static double g_totalNormalDelay = 0.0, g_totalHpDelay = 0.0;
static std::string g_per_step_info = "";

void
ConsumeJouleEnergy(uint32_t n, double nj)
{
    if (g_nodesState[n].residualEnergy > 0)
    {
        g_nodesState[n].residualEnergy =
            std::max(0.0, g_nodesState[n].residualEnergy - (nj * 1e-9));
    }
}

void
ConsumeBitEnergy(uint32_t n, double b, double d)
{
    if (g_nodesState[n].residualEnergy > 0)
    {
        double c = E_ELEC * b;
...
UpdateNetworkTopology()
{
    for (uint32_t i = 0; i < SENSOR_NODES; ++i)
    {
        ...
        {
            if (i != j && g_allNodes.Get(i)->GetObject<MobilityModel>()->GetDistanceFrom(
                              g_allNodes.Get(j)->GetObject<MobilityModel>()) < 40.0)
...
GetGameOver()
{
    ...
    if (g_sim_step_count >= MAX_SIM_STEPS)
    {
        ...
        double pdr = (g_packetsGenerated > 0)
                         ? ((double)g_packetsDelivered / g_packetsGenerated) * 100.0
                         : 0.0;
        double plr = (g_packetsGenerated > 0)
                         ? ((double)g_packetsDropped / g_packetsGenerated) * 100.0
                         : 0.0;
        double nd =
            (g_normalPacketsDelivered > 0) ? (g_totalNormalDelay / g_normalPacketsDelivered) : 0.0;
        double hd = (g_hpPacketsDelivered > 0) ? (g_totalHpDelay / g_hpPacketsDelivered) : 0.0;
        double te = 0;
        for (uint32_t j = 0; j < SENSOR_NODES; ++j)
            te += g_nodesState[j].residualEnergy;
        double ec = (SENSOR_NODES * INITIAL_ENERGY) - te;
        double eff =
            (g_packetsDelivered > 0) ? (ec * 1e9) / (g_packetsDelivered * PACKET_BITS) : 0.0;
        std::stringstream ss;
        ss << "{\"throughput_kbps\":" << t << ",\"pdr_pct\":" << pdr << ",\"plr_pct\":" << plr
           << ",\"avg_normal_delay_s\":" << nd << ",\"avg_hp_delay_s\":" << hd
           << ",\"energy_nj_bit\":" << eff << "}";
...
GetObservation()
{
    ...
    while (!g_nodesState[g_currentCH].hp_q.empty() &&
           (Simulator::Now() - g_nodesState[g_currentCH].hp_q.front().enqueueTime).GetSeconds() >
               HP_TIMEOUT)
    {
    ...
    while (
        !g_nodesState[g_currentCH].normal_q.empty() &&
        (Simulator::Now() - g_nodesState[g_currentCH].normal_q.front().enqueueTime).GetSeconds() >
            NORMAL_TIMEOUT)
    {
    ...
    for (int i = 0; i < numPackets; ++i)
    {
        if (g_rand->GetValue() < 0.7)
        {
            ...
            if (attempts < 100)
            {
                ...
                double dist = g_allNodes.Get(taskNode)->GetObject<MobilityModel>()->GetDistanceFrom(
                    g_allNodes.Get(g_currentCH)->GetObject<MobilityModel>());
    ...
    double n_age = g_nodesState[g_currentCH].normal_q.empty()
                       ? 0.0
                       : (Simulator::Now() - g_nodesState[g_currentCH].normal_q.front().enqueueTime)
                             .GetSeconds();
    double hp_age =
        g_nodesState[g_currentCH].hp_q.empty()
            ? 0.0
            : (Simulator::Now() - g_nodesState[g_currentCH].hp_q.front().enqueueTime).GetSeconds();
    double dist_to_sink =
        g_allNodes.Get(g_currentCH)
            ->GetObject<MobilityModel>()
            ->GetDistanceFrom(g_allNodes.Get(SINK_NODE_ID)->GetObject<MobilityModel>());
    ...
    box->AddValue(std::min(1.0, dist_to_sink / 158.0));
    return box;
}

bool
ExecuteActions(Ptr<OpenGymDataContainer> a)
{
    ...
    {
        for (int i = 0; i < 3 && !g_nodesState[g_currentCH].normal_q.empty(); ++i)
        {
            PacketInfo p = g_nodesState[g_currentCH].normal_q.front();
            g_nodesState[g_currentCH].normal_q.pop_front();
            g_totalNormalDelay += (Simulator::Now() - p.enqueueTime).GetSeconds();
    ...
    current_info.pop_back();
    std::stringstream ss;
    ss << ",\"hp_sent\":" << hp_s << ",\"normal_sent\":" << n_s << ",\"hp_dropped\":" << hp_d
       << ",\"normal_dropped\":" << n_d << "}";
    current_info += ss.str();
...
void
ScheduleNextStep(uint32_t i, uint32_t& c)
{
    if (GetGameOver())
    {
    ...
    if ((c > 0 && g_nodesState[g_currentCH].residualEnergy < INITIAL_ENERGY * 0.05) ||
        (c > 0 && c % 20 == 0))
        ElectClusterHead();
    c++;
...
main(int argc, char* argv[])
{
    ...
    MobilityHelper senm;
    senm.SetPositionAllocator("ns3::RandomRectanglePositionAllocator",
                              "X",
