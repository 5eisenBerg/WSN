...
# --- V74 Config: The Final, Unified Script ---
CURRENT_DIR = os.path.dirname(os.path.abspath(__file__))
DATA_DIR = os.path.join(CURRENT_DIR, 'data_v74')
FIG_DIR = os.path.join(CURRENT_DIR, 'figures_v74')
...
PORT = 5555
MODEL_PATH = os.path.join(CURRENT_DIR, "v74_best.pth")
...
# V74: Final, Balanced, Outcome-Driven Reward Function
...
def calculate_reward_v74(s, ps, info):
...
def run_simulation(policy, steps, train_mode=False, run_id=1, agent=None, seed=BASE_SEED, collect_data=False):
    ...
    try:
        ...
        for step in bar:
            ...
            if train_mode:
                r = 0 if d else calculate_reward_v74(s_next, ps, info)
                rewards_log.append(r)
                agent.remember(s, a, r, s_next, d)
                agent.train()
            s = s_next
            if d:
                if "throughput_kbps" not in info:
                    raise RuntimeError("Simulator did not return final metrics.")
                final_info = info
                break
        if collect_data:
            os.makedirs(DATA_DIR, exist_ok=True)
            if train_mode:
                np.savetxt(os.path.join(DATA_DIR, f'reward_log_epoch{run_id}.txt'), rewards_log)
            for fname in ['hp_delays.txt', 'normal_delays.txt', 'queue_log.txt', 'energy_log.txt']:
                src = os.path.join(NS3_PATH, fname)
                dst = os.path.join(DATA_DIR, f'{policy}_{fname}')
...
def main_experiment():
    ...
    for epoch in range(TRAINING_EPOCHS):
        ...
        for i, val_seed in enumerate(VALIDATION_SEEDS):
            ...
            if val_res:
                val_scores.append(get_eval_score(val_res))
        if not val_scores:
            raise RuntimeError(f"Validation failed for epoch {epoch + 1}.")
        ...
        if epoch - best_epoch >= 15:
            ...
            break
    pd.DataFrame(train_log).to_csv("training_log_v74.csv", index=False)
    print("\n📊 V74: Final Evaluation...")
    raw_results = []
    for i in range(20):
    ...
    raw_df.to_csv('raw_results_v74.csv', index=False)
    summary_data = []
    for policy, group in raw_df.groupby('Policy'):
        entry = {'Policy': policy}
        for col in group.columns:
            if col in ["Policy", "run", "action"]:
                continue
            mean = group[col].mean()
            ci = stats.t.ppf(0.975, len(group) - 1) * group[col].std(ddof=1) / np.sqrt(len(group))
            entry[col] = f"{mean:.4f}±{ci:.4f}"
        summary_data.append(entry)
    summary_df = pd.DataFrame(summary_data).set_index('Policy')
    print("\n\n--- FINAL RESULTS (V74, MEAN ± 95% CI) ---")
    print(summary_df.to_string())
    summary_df.to_excel("final_results_v74.xlsx")
...
def plot_all_figures():
    ...
    try:
        raw_df = pd.read_csv('raw_results_v74.csv')
        log_df = pd.read_csv('training_log_v74.csv')
    except FileNotFoundError:
        print("❌ Data files not found. Cannot generate plots.")
        return
    # Fig 1 & 2: Learning and Reward Curves
    try:
        fig, ax1 = plt.subplots(figsize=(12, 7))
        ax2 = ax1.twinx()
        ax1.plot(log_df['epoch'], log_df['score'].rolling(5, min_periods=1, center=True).mean(), 'b-', label='Validation Score (5-Epoch Avg)', linewidth=3)
        ax2.plot(log_df['epoch'], log_df['epsilon'], 'r--', label='Epsilon Decay', alpha=0.6)
        ax1.set_xlabel('Training Epoch')
        ax1.set_ylabel('Validation Score', color='b')
        ax2.set_ylabel('Epsilon', color='r')
        best_idx = log_df['score'].idxmax()
        best_score = log_df['score'].max()
        best_epoch = log_df['epoch'][best_idx]
        ax1.axvline(x=best_epoch, color='g', linestyle='--', label=f'Best Model @ Epoch {best_epoch}')
        ax1.scatter(best_epoch, best_score, color='g', s=150, zorder=5)
        plt.title('Agent Training Progression', fontsize=18, fontweight='bold')
        fig.legend(loc="upper right", bbox_to_anchor=(0.9, 0.9))
        plt.tight_layout()
        plt.savefig(os.path.join(FIG_DIR, 'fig_1_learning_curve.png'), dpi=300, bbox_inches="tight")
        plt.close()
        print("✅ Fig 1: Learning Curve")
        rewards = np.loadtxt(os.path.join(DATA_DIR, 'reward_log_epoch1.txt'))
        reward_df = pd.DataFrame({'step': range(len(rewards)), 'reward': rewards})
        reward_df['reward_smooth'] = reward_df['reward'].rolling(window=100, min_periods=1).mean()
        plt.figure(figsize=(12, 7))
        plt.plot(reward_df['step'], reward_df['reward_smooth'])
        plt.title('Per-Step Reward (First Training Epoch)', fontsize=18, fontweight='bold')
        plt.xlabel('Training Step')
        plt.ylabel('Smoothed Reward (100-step avg)')
        plt.savefig(os.path.join(FIG_DIR, 'fig_2_reward_curve.png'), dpi=300, bbox_inches="tight")
        plt.close()
        print("✅ Fig 2: Reward Curve")
    except Exception as e:
        print(f"❌ Error in training plots: {e}")

    # Fig 3 & 4: Boxplots and Bar Charts
    df_melted = raw_df.melt(id_vars='Policy', value_vars=METRICS_FOR_STATS.keys(), var_name='Metric', value_name='Value')
    df_melted['Metric'] = df_melted['Metric'].map(METRICS_FOR_STATS)
    g = sns.catplot(data=df_melted, kind='box', x='Policy', y='Value', col='Metric', col_wrap=2, height=5, aspect=1.3, sharey=False, order=list(POLICIES.keys()))
    g.fig.suptitle('Distribution of Performance Metrics (20 Runs)', y=1.03, fontsize=18, fontweight='bold')
    g.set_axis_labels("", "Value")
    g.set_titles("{col_name}")
    plt.tight_layout(rect=[0, 0, 1, 0.96])
    plt.savefig(os.path.join(FIG_DIR, 'fig_3_boxplots.png'), dpi=300, bbox_inches="tight")
    plt.close()
    print("✅ Fig 3: Boxplots")
    g = sns.catplot(data=df_melted, kind='bar', x='Policy', y='Value', col='Metric', col_wrap=2, height=5, aspect=1.3, sharey=False, order=list(POLICIES.keys()), errorbar=None)
    g.fig.suptitle('Mean Performance Comparison (95% CI)', y=1.03, fontsize=18, fontweight='bold')
    g.set_axis_labels("", "Mean Value")
    g.set_titles("{col_name}")
    for ax, metric in zip(g.axes.flat, METRICS_FOR_STATS.values()):
        sub_df = df_melted[df_melted['Metric'] == metric]
        means = sub_df.groupby('Policy')['Value'].mean().loc[list(POLICIES.keys())]
        cis = sub_df.groupby('Policy')['Value'].apply(lambda x: stats.t.ppf(0.975, len(x) - 1) * x.std(ddof=1) / np.sqrt(len(x))).loc[list(POLICIES.keys())]
        ax.errorbar(x=range(len(means)), y=means, yerr=cis, fmt='none', c='black', capsize=5)
    plt.tight_layout(rect=[0, 0, 1, 0.96])
    plt.savefig(os.path.join(FIG_DIR, 'fig_4_bar_charts.png'), dpi=300, bbox_inches="tight")
    plt.close()
    print("✅ Fig 4: Bar Charts")

    # Fig 5 & 6: Delay CDF and Queue Violin
    plt.figure(figsize=(12, 7))
    ...
    plt.savefig(os.path.join(FIG_DIR, 'fig_5_delay_cdf.png'), dpi=300, bbox_inches="tight")
    plt.close()
    print("✅ Fig 5: Delay CDF")
    all_q_data = []
    for label, p_short in POLICIES.items():
        try:
            df = pd.read_csv(os.path.join(DATA_DIR, f'{p_short}_queue_log.txt'), names=['time', 'hp', 'normal'])
            df['Policy'] = label
            all_q_data.append(df)
        except FileNotFoundError:
            continue
    if all_q_data:
        q_df = pd.concat(all_q_data)
        plt.figure(figsize=(12, 7))
        sns.violinplot(data=q_df, x='Policy', y='normal', order=list(POLICIES.keys()), inner='box', cut=0)
        plt.title('Distribution of Normal Queue Occupancy', fontsize=18, fontweight='bold')
        plt.xlabel('Policy')
        plt.ylabel('Queue Length (packets)')
        plt.savefig(os.path.join(FIG_DIR, 'fig_6_queue_violin.png'), dpi=300, bbox_inches="tight")
        plt.close()
        print("✅ Fig 6: Queue Violin Plot")

    # Fig 7 & 8: Action Distribution and Radar Chart
    try:
        actions = np.loadtxt(os.path.join(DATA_DIR, 'DQN_actions.txt'), dtype=int)
        counts = pd.Series(actions).value_counts().reindex(range(len(ACTION_LABELS)), fill_value=0)
        percentages = counts / counts.sum() * 100
        plt.figure(figsize=(10, 6))
        ax = sns.barplot(x=ACTION_LABELS, y=percentages, palette='viridis')
        ax.bar_label(ax.containers[0], fmt='%.1f%%')
        plt.title('DQN Agent Learned Policy', fontsize=18, fontweight='bold')
        plt.xlabel('Action')
        plt.ylabel('Policy Usage (%)')
        plt.ylim(0, 1.1 * percentages.max())
        plt.savefig(os.path.join(FIG_DIR, 'fig_7_action_dist.png'), dpi=300, bbox_inches="tight")
        plt.close()
        print("✅ Fig 7: Action Distribution")
    except FileNotFoundError:
        print("❌ Action log file not found. Skipping plot.")
    summary_df = raw_df.groupby('Policy').mean().reset_index()
    metrics_radar = ['pdr_pct', 'throughput_kbps', 'avg_hp_delay_s', 'energy_nj_bit']
    labels_radar = ['PDR (%) ↑', 'Throughput (kbps) ↑', 'HP Delay (s) ↓', 'Energy (nJ/bit) ↓']
    df_norm = summary_df.copy()
    for metric in metrics_radar:
        min_val, max_val = df_norm[metric].min(), df_norm[metric].max()
        if metric in ['avg_hp_delay_s', 'energy_nj_bit']:
    ...
    for i, row in df_norm.iterrows():
        if row['Policy'] not in POLICIES.keys():
            continue
        data = row[metrics_radar].values.flatten().tolist()
        data += data[:1]
    ...
    plt.savefig(os.path.join(FIG_DIR, 'fig_8_radar_chart.png'), dpi=300, bbox_inches="tight")
    plt.close()
