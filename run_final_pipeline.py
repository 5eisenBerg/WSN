...
NS3_PATH='/home/heisenberg/ns3-workspace/bake/source/ns-3.40';SIM_SCRIPT='wsn_100dynamic';PORT=5555;MODEL_PATH=os.path.join(CURRENT_DIR,"v74_best.pth")
STATE_SIZE,ACTION_SIZE=6,5;TRAINING_EPOCHS,TRAIN_STEPS_PER_EPOCH,EVAL_STEPS=50,5000,5000
BASE_SEED=12345;VALIDATION_SEEDS=[99991,99992,99993,99994,99995]

# V74: Final, Balanced, Outcome-Driven Reward Function
HP_DELIVERY_REWARD=25.0;NORMAL_DELIVERY_REWARD=5.0
HP_DROP_PENALTY=-40.0;NORMAL_DROP_PENALTY=-8.0
HP_TIMEOUT_PENALTY=-80.0;NORMAL_TIMEOUT_PENALTY=-10.0
HP_LATENCY_PENALTY=-40.0;NORMAL_LATENCY_PENALTY=-2.0
ENERGY_PENALTY_WEIGHT=250.0;SLEEP_REWARD=0.5
EPSILON_DECAY=0.99995;WARMUP_STEPS=4000
...
class PER_DoubleDQNAgent:
    def __init__(self,tau=0.005,alpha=0.6,beta_start=0.4,beta_frames=100000):
        ...
        self.optimizer=optim.Adam(self.model.parameters(),lr=1e-4);self.memory=PrioritizedReplayBuffer(100000,alpha);self.epsilon=1.0;self.train_steps=0;self.tau=tau
    ...
    def train(self):
        if len(self.memory)<WARMUP_STEPS:return
        ...
        if len(self.memory)>WARMUP_STEPS:self.epsilon=max(0.05,self.epsilon*EPSILON_DECAY)
...
def calculate_reward_v74(s,ps,info):
    ...
    if info.get('action')==3:reward+=SLEEP_REWARD
    return np.tanh(reward/100.0) # Adjusted tanh scaling
...
def run_simulation(policy,steps,train_mode=False,run_id=1,agent=None,seed=BASE_SEED,collect_data=False):
    ...
    try:
        ...
        for step in bar:
            ...
            if train_mode:r=0 if d else calculate_reward_v74(s_next,ps,info);rewards_log.append(r);agent.remember(s,a,r,s_next,d);agent.train()
            s=s_next
            if d:
...
def get_eval_score(res):
    ...
    return np.clip((0.6*pdr/100.0)+(0.1*(1-plr/100.0))+(0.3*(1-min(hd/1.0,1.0))),0.0,1.0) # Score now prioritizes PDR more

def main_experiment():
    print("🛠️ V74: Rebuilding NS-3...");build=subprocess.run(f"./ns3 build",cwd=NS3_PATH,shell=True,capture_output=True,text=True)
    ...
    for epoch in range(TRAINING_EPOCHS):
        ...
        mean_score=np.mean(val_scores);print(f"Epoch {epoch+1} Mean Val Score: {mean_score:.4f} (across {len(VALIDATION_SEEDS)} seeds)")
        ...
        if mean_score>best_eval_score:best_eval_score=mean_score;best_epoch=epoch;training_agent.save(MODEL_PATH);print(f"  🥇 New best model saved (Score: {best_eval_score:.4f})")
    ...
    print("\n📊 V74: Final Evaluation...");raw_results=[]
    for i in range(20):
    ...
    raw_df=pd.DataFrame(raw_results);raw_df.to_csv('raw_results_v74.csv',index=False)
    summary_data=[]
    ...
    summary_df=pd.DataFrame(summary_data).set_index('Policy');print("\n\n--- FINAL RESULTS (V74, MEAN ± 95% CI) ---");print(summary_df.to_string())
    summary_df.to_excel("final_results_v74.xlsx");print("\n✅ Raw and summary results exported.")
...
def plot_all_figures():
    ...
    summary_df_agg=raw_df.groupby('Policy').agg(['mean','std']).reset_index();summary_df_agg.columns=[' '.join(col).strip() for col in summary_df_agg.columns.values];policy_order=list(POLICIES.keys());summary_df_agg=summary_df_agg.set_index('Policy').loc[policy_order].reset_index()
