...
PORT = 5555
MODEL_PATH = os.path.join(CURRENT_DIR, "v54_best.pth")
STATE_SIZE, ACTION_SIZE = 10, 11
TRAINING_EPOCHS, TRAIN_STEPS_PER_EPOCH, EVAL_STEPS, BASE_SEED = 40, 2500, 5000, 12345
...
def calculate_reward(s, ps, info):
    ...
    if ec > 0:
        r -= ec * 5000
    if info.get('action') == 10:  # V54: Sleep is now action 10
        r += SLEEP_PENALTY
    ...
    try:
        ...
        for step in bar:
            ...
            if train_mode and len(agent.memory) < agent.warmup:
                ...
                dist_to_sinks = s[6:]
                closest_sink_idx = np.argmin(dist_to_sinks)
                if s[1] > 0: a = closest_sink_idx
                elif s[0] > 0: a = 4 + closest_sink_idx
                else: a = 10
            else:
                a = np.random.randint(ACTION_SIZE)
...
def get_eval_score(res):
    t,p,plr,nd,hd,e=res.get('throughput_kbps',0),res.get('pdr_pct',0),res.get('plr_pct',0),res.get('avg_normal_delay_s',5),res.get('avg_hp_delay_s',2),res.get('energy_nj_bit',10000)
    norm_pdr = p / 100.0 if p is not None else 0
    norm_hd = 1.0 - min(hd / 2.0, 1.0) if hd is not None else 0
    score = 0.7 * norm_pdr + 0.3 * norm_hd
    return np.clip(score, 0.0, 1.0)

if __name__ == '__main__':
    print("🛠️ V54: Rebuilding NS-3...")
    ...
    best_eval_score = float("-inf"); best_epoch = 0; train_log = []
    
    print("\n🧠 V54: Training Final DQN...")
    ...
    pd.DataFrame(train_log).to_csv("training_log_v54.csv", index=False)
    
    print("\n📊 V54: Final Evaluation...")
    ...
    print("\n\n--- FINAL RESULTS (V54, MEAN ± 95% CI) ---")
    print(summary_df.to_string())
    summary_df.to_excel("final_results_v54.xlsx")
    print("\n✅ Results exported to 'final_results_v54.xlsx'")
    ...
    if results["Proposed DQN-Edge"] and results["Strict Priority"]:
        for metric in results["Proposed DQN-Edge"][0].keys():
            ...
            if n > 1:
                t, p_val = stats.ttest_rel(dqn_data[:n], sp_data[:n])
