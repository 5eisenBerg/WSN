...
# --- V58 Config ---
# Using the stable V57 C++ simulator with a focused Module 1 setup
...
NS3_PATH='/home/heisenberg/ns3-workspace/bake/source/ns-3.40';SIM_SCRIPT='wsn_100dynamic';PORT=5555;MODEL_PATH=os.path.join(CURRENT_DIR,"v58_best.pth")
STATE_SIZE,ACTION_SIZE=6,5;TRAINING_EPOCHS,TRAIN_STEPS_PER_EPOCH,EVAL_STEPS,BASE_SEED=30,2500,5000,12345
...
# --- RL Agent and Simulation Functions (identical to V55/V57, proven stable) ---
...
def calculate_reward(s,ps,info):
    r=info.get('hp_sent',0)*HP_REWARD+info.get('normal_sent',0)*NORMAL_REWARD+info.get('hp_dropped',0)*DROP_HP_PENALTY+info.get('normal_dropped',0)*DROP_NORMAL_PENALTY
    r+=info.get('hp_timeout',0)*DROP_HP_PENALTY*1.2+info.get('normal_timeout',0)*DROP_NORMAL_PENALTY*1.2
    r+=HP_DELAY_WEIGHT*s[4]+NORMAL_DELAY_WEIGHT*s[3];ec=ps[2]-s[2]
    if ec>0:r-=ec*5000
    if info.get('action')==3:r+=SLEEP_PENALTY
...
def run_simulation(policy,steps,train_mode=False,run_id=1,agent=None,seed=BASE_SEED,collect_data=False):
    ...
    env=None;original_epsilon=None;actions_log=[]
    try:
        ...
        for step in bar:
            ...
            if train_mode:r=0 if d else calculate_reward(s_next,ps,info);agent.remember(s,a,r,s_next,d);agent.train()
            s=s_next
            if d:assert "throughput_kbps" in info;final_info=info;break
        if collect_data:
            datadir='data_v58';os.makedirs(datadir,exist_ok=True)
            for fname in ['hp_delays.txt','normal_delays.txt','queue_log.txt','energy_log.txt']:
                if os.path.exists(os.path.join(NS3_PATH,fname)):shutil.copy(os.path.join(NS3_PATH,fname),os.path.join(datadir,f'{policy}_{fname}'))
...
def main_experiment():
    print("🛠️ V58: Rebuilding NS-3...");build=subprocess.run(f"./ns3 build",cwd=NS3_PATH,shell=True,capture_output=True,text=True)
    ...
    print("\n📊 V58: Final Evaluation...");results={"Proposed DQN-Edge":[],"Strict Priority":[],"Random":[]}
    ...
    for p,r in results.items():
        if not r:continue
        df=pd.DataFrame(r);final[p]={k:f"{df[k].mean():.4f}±{stats.t.ppf(0.975,len(df)-1)*df[k].std()/np.sqrt(len(df)):.4f}"for k in df.columns if k !='action'}
    summary_df=pd.DataFrame(final).T;print("\n\n--- FINAL RESULTS (V58, MEAN ± 95% CI) ---");print(summary_df.to_string())
    summary_df.to_excel("final_results_v58.xlsx");print("\n✅ Results exported to 'final_results_v58.xlsx'")

def plot_all_figures():
    print("\n\n--- 📈 Generating Publication Plots ---")
    sns.set_theme(style="whitegrid",palette="muted",font_scale=1.2);datadir='data_v58'
    # ... (Plotting functions will be called here)
