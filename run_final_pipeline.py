...
NS3_PATH = '/home/heisenberg/ns3-workspace/bake/source/ns-3.40'; SIM_SCRIPT = 'wsn_100dynamic'; PORT = 5555; MODEL_PATH = os.path.join(CURRENT_DIR, "v51_best.pth")
STATE_SIZE, ACTION_SIZE = 7, 5; TRAINING_EPOCHS, TRAIN_STEPS_PER_EPOCH, EVAL_STEPS, BASE_SEED = 30, 2000, 5000, 12345
...
class PER_DoubleDQNAgent:
    def __init__(self, tau=0.005, warmup=1000, alpha=0.6, beta_start=0.4, beta_frames=100000):
        self.model = DQN(STATE_SIZE, ACTION_SIZE); self.target = DQN(STATE_SIZE, ACTION_SIZE)
        self.target.load_state_dict(self.model.state_dict()); self.target.eval()
        self.optimizer = optim.Adam(self.model.parameters(), lr=1e-4); self.memory = PrioritizedReplayBuffer(100000, alpha)
        self.epsilon = 1.0; self.train_steps = 0; self.tau = tau; self.warmup = warmup
    ...
    try:
        ...
        for step in bar:
            ...
            if train_mode:
                ...
                agent.train()
            s=s_next
            if d:assert "throughput_kbps" in info, "Final metrics not in terminal info";final_info=info;break
...
if __name__=='__main__':
    print("🛠️ V51: Rebuilding NS-3...");build=subprocess.run(f"./ns3 build",cwd=NS3_PATH,shell=True,capture_output=True,text=True)
    ...
    pd.DataFrame(train_log).to_csv("training_log_v51.csv",index=False)
    print("\n📊 V51: Final Evaluation...");results={"Proposed DQN-Edge":[],"Strict Priority":[],"Random":[]}
    ...
    summary_df=pd.DataFrame(final).T;print("\n\n--- FINAL RESULTS (V51, MEAN ± 95% CI) ---");print(summary_df.to_string())
    summary_df.to_excel("final_results_v51.xlsx");print("\n✅ Results exported to 'final_results_v51.xlsx'")
