...
NS3_PATH = '/home/heisenberg/ns3-workspace/bake/source/ns-3.40'; SIM_SCRIPT = 'wsn_100dynamic'; PORT = 5555; MODEL_PATH = os.path.join(CURRENT_DIR, "v54_best.pth")
STATE_SIZE, ACTION_SIZE = 10, 11 # V54: Updated for multi-sink
...
class PER_DoubleDQNAgent:
    # ... (Agent class is the same, but re-initialized with new STATE/ACTION sizes)
...
def calculate_reward(s,ps,info):
    ...
    ec=ps[2]-s[2];
    if ec>0:r-=ec*5000
    if info.get('action')==10:r+=SLEEP_PENALTY # Action 10 is now Sleep
    ...
    try:
        ...
        for step in bar:
            ...
            if train_mode and len(agent.memory)<agent.warmup:a=np.random.randint(ACTION_SIZE)
                ...
                # Send to the closest sink
                dist_to_sinks = s[6:]
                closest_sink_idx = np.argmin(dist_to_sinks)
                if s[1]>0: a = closest_sink_idx # Send HP to closest
                elif s[0]>0: a = 4 + closest_sink_idx # Send Normal to closest
                else: a = 10 # Sleep
...
if __name__=='__main__':
    print("🛠️ V54: Rebuilding NS-3...");build=subprocess.run(f"./ns3 build",cwd=NS3_PATH,shell=True,capture_output=True,text=True)
