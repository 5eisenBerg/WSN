...
using namespace ns3;
NS_LOG_COMPONENT_DEFINE("WSN_Final_V31");
...
bool ExecuteActions(Ptr<OpenGymDataContainer> a) {
    ...
    // --- BUG FIX: Get mobility model of sink node ---
    double dist_to_sink = g_allNodes.Get(g_currentCH)->GetObject<MobilityModel>()->GetDistanceFrom(g_allNodes.Get(SINK_NODE_ID)->GetObject<MobilityModel>());
...
void ScheduleNextStep(uint32_t i, uint32_t& c) {
    if (g_gym->IsGameOver()) { g_gym->NotifySimulationEnd(); return; }
    // Corrected parentheses for clarity
...
int main(int argc, char* argv[]) {
    uint32_t port = 5555; uint32_t run = 1;
    CommandLine cmd; cmd.AddValue("openGymPort", "Port", port); cmd.AddValue("run", "Run", run); cmd.Parse(argc, argv);
    SeedManager::SetSeed(12345); SeedManager::SetRun(run);
    ...
    MobilityHelper senm; senm.SetPositionAllocator("ns3::RandomRectanglePositionAllocator", "X", StringValue("ns3::UniformRandomVariable[Min=0|Max=100]"), "Y", StringValue("ns3::UniformRandomVariable[Min=0|Max=100]")); senm.SetMobilityModel("ns3::ConstantPositionMobilityModel");
    NodeContainer sensors; for (uint32_t i = 0; i < SENSOR_NODES; ++i) sensors.Add(g_allNodes.Get(i));
