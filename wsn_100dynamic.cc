...
static uint64_t g_normalPacketsDelivered = 0, g_hpPacketsDelivered = 0;
...
Ptr<OpenGymDataContainer> GetObservation() {
    ...
    for (int i = 0; i < numPackets; ++i) {
        if (g_rand->GetValue() < 0.45) { // Approx 0.9 packets/step
    ...
    box->AddValue(std::min(1.0, age / 10.0));
...
std::string GetExtraInfo() {
    ...
    std::stringstream ss;
    ss << "{\"throughput_kbps\":" << t << ",\"pdr_pct\":" << pdr << ",\"plr_pct\":" << plr << ",\"avg_normal_delay_s\":" << nd << ",\"avg_hp_delay_s\":" << hd << ",\"energy_nj_bit\":" << eff << "}";
    return ss.str();
}
